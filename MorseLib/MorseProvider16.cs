...
namespace MorseLib
{
    public class MorseProvider16 : WaveProvider16
    {
        ...
        /// 基本点占的时长(毫秒）
        /// </summary>
        public int BaseDotMiSeconds
        {
            get => _baseDotMiSeconds; set
            {
        ...
        public override int Read(short[] buffer, int offset, int sampleCount)
        {
            ...
            while (_decodesBuf.Count < sampleCount && _decodeWaveIndex < _morseCode.Codes.Length)
            {
                ...
                if (OnPlaying != null)
                {
                    var idx = _decodeWaveIndex;
                    Task.Run(() =>
                    {
                        for (var i = 0; i < _morseCode.CharInCodesRange.Count; i++)
                        {
                            if (idx <= _morseCode.CharInCodesRange[i].End)
                            {
                                var tmp = i;
                                OnPlaying.Invoke(_morseCode.Text, tmp);
                                break;
                            }    
                        }
                    });
